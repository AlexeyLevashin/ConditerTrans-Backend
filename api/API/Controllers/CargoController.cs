...
public class CargoController(ICargoService cargoService) : BaseController
{
    ...
    {
        var result = await cargoService.GetPendingForCoordinatorAsync(UserId, UserRole, page, pageSize);
    ...
    public async Task<IActionResult> GetActiveForCoordinator(
        ...
        [FromQuery] int pageSize = 20)
    {
        var result = await cargoService.GetActiveForCoordinatorAsync(
            UserId,
            UserRole,
            CompanyId,
            page,
    ...
    [HttpGet("coordinator")]
    public async Task<IActionResult> GetAllForCoordinator(
        [FromQuery] CargoStatus? status,
        [FromQuery] int page = 1,
        [FromQuery] int pageSize = 20)
    {
        var result = await cargoService.GetAllForCoordinatorAsync(
            UserId,
            UserRole,
            CompanyId,
            status,
    ...
    public async Task<IActionResult> GetActiveForDriver(
        ...
        [FromQuery] int pageSize = 20)
    {
        var result = await cargoService.GetActiveForDriverAsync(UserId, UserRole, page, pageSize);
